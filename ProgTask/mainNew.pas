...
begin
  ...
  begin
    if MessageDlg('Нет соединения с базой данных, закрыть программу?', mtConfirmation, mbYesNo, 0) =  mrYes  then
    begin
      Close;
    end
  end;
...
begin
  ...
  begin
    ShowMessage('Все поля заявки должны быть заполнены!');
    exit;
  end;
  ...
  if MessageDlg('Ваша заявка'+s+' принята, закрыть программу?', mtConfirmation, mbYesNo, 0) =  mrYes  then
  begin
    Close;
  end
  else
